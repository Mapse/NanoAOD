...
das_path = 'charmonium_dataset_2018.txt'
...
for dataset in datasets:
    ...
    # Stores workarea name
    WORKAREA = PRIM_DATASET + ERA[0:-1] + "UL"
    # Stores directory name. Ex: 'data18UL'
    OUT_DIR = "Data" + ERA[-3:-1] + "UL"
    # Stores request name. Ex : 'CharmoniumRun2018A_AOD'
    REQUEST_NAME = PRIM_DATASET + ERA + "_" + splited[-1]
    ...
    with open(crab_template + ".py", 'r') as f:
        ...
        new_file = new_file.replace("CONFIG_NAME", config_template + "_" + ERA)

    with open(crab_template + "_" + ERA + ".py", 'w') as nf:
        nf.write(new_file)

    with open(config_template + ".py", 'r') as f:
        new_file = f.read().replace("REQUEST_NAME", REQUEST_NAME)

    with open(config_template + "_" + ERA + ".py", 'w') as nf:
        nf.write(new_file)

    os.system("crab submit -c " + crab_template + "_" + ERA + ".py")
